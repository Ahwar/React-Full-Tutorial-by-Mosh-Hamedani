...
class Movies extends Component {
  ...
  renderMoviesInTable() {
    return this.state.movies.map(movie => (
      <tr key={movie._id}>
        ...
        <td>
          <Like liked={movie.liked} onClicked={() => this.handleLike(movie)} />
        </td>
        <td>
          <button
  ...
  handleLike(movieObj) {
    let mvs = [...this.state.movies];
    let idx = mvs.indexOf(movieObj);
    mvs[idx].liked = !mvs[idx].liked;
